...
:- dynamic rTTIBaseClassDescriptor/8 as incremental.
...
rTTISelfRef(TDA, COLA, CHDA, BCDA, VFTable, Name) :-
    ...
    % Debugging.
    %write('debug-rTTISelfRef('),
    %write(TDA), write(', '),
    %write(COLA), write(', '),
    %write(CHDA), write(', '),
    %write(BCDA), write(', '),
    %write(VFTable), write(', '),
    %write('\''), write(Name), write('\''), writeln(').'),
    true.
...
rTTIInheritsDirectlyFrom(DerivedTDA, AncestorTDA, Attributes, M, P, V) :-
    ...
    %write('debug-rTTIInheritsDirectlyFrom('),
    %write(DerivedTDA), write(', '),
    %write(AncestorTDA), write(', '),
...
rTTIInheritsVirtuallyFrom(DerivedTDA, AncestorTDA, Attributes, M, P, V) :-
    ...
    % Debugging.
    %write('debug-rTTIInheritsVirtuallyFrom('),
    %write(DerivedTDA), write(', '),
    %write(AncestorTDA), write(', '),
    %write(Attributes), write(', '),
    %write(M), write(', '),
...
reportNoBase((A)) :-
    ...
    write(A), write(', '),
    rTTIName(A, AName),
    write('\''), write(AName), write('\''), writeln(').').
reportNoBase :-
