...
:- index(preceeds/2, [1,2]).
:- dynamic preceeds/2 as incremental.
:- index(funcOffset/4, [1,2,3,4]).
:- dynamic funcOffset/4 as incremental.
:- index(methodMemberAccess/4, [1,2,3,4]).
:- dynamic methodMemberAccess/4 as incremental.
:- dynamic possibleConstructor/1 as incremental.
:- dynamic uninitializedReads/1 as incremental.
:- index(possibleVFTableWrite/4, [1,2,3,4]).
:- dynamic possibleVFTableWrite/4 as incremental.
:- index(possibleVFTableEntry/3, [1,2,3]).
:- dynamic possibleVFTableEntry/3 as incremental.
:- index(possibleVBTableWrite/4, [1,2,3,4]).
:- dynamic possibleVBTableWrite/4 as incremental.
:- index(possibleVBTableEntry/3, [1,2,3]).
:- dynamic possibleVBTableEntry/3 as incremental.
:- index(possibleVirtualFunctionCall/5, [1,2,3,4,5]).
:- dynamic possibleVirtualFunctionCall/5 as incremental.
:- index(rTTICompleteObjectLocator/6, [1,2,3,4,5,6]).
:- dynamic rTTICompleteObjectLocator/6 as incremental.
:- index(rTTITypeDescriptor/3, [1,2,3]).
:- dynamic rTTITypeDescriptor/3 as incremental.
:- index(rTTIClassHierarchyDescriptor/3, [1,2,3]).
:- dynamic rTTIClassHierarchyDescriptor/3 as incremental.
:- index(rTTIBaseClassDescriptor/7, [1,2,3,4,5,6,7]).
:- dynamic rTTIBaseClassDescriptor/7 as incremental.
:- index(thisPtrAllocation/5, [1,2,3,4,5]).
:- dynamic thisPtrAllocation/5 as incremental.
:- index(thisPtrUsage/4, [1,2,3,4]).
:- dynamic thisPtrUsage/4 as incremental.
:- index(thisPtrOffset/3, [1,2,3]).
:- dynamic thisPtrOffset/3 as incremental.
:- dynamic purecall/1 as incremental.
:- dynamic insnCallsDelete/1 as incremental.
:- index(symbolGlobalObject/3, [1,2,3]).
:- dynamic symbolGlobalObject/3 as incremental.
:- index(symbolClass/3, [1,2,3]).
:- dynamic symbolClass/3 as incremental.
:- index(symbolProperty/2, [1,2]).
:- dynamic symbolProperty/2 as incremental.
...
% Uncommenting the following line will cause all our prolog rules to be placed into a
% monolothic file, setup.pl_gpp.
%:- compiler_options([xpp_dump]).
...
try_assert_real(X) :-
    %debug('Retracting '), debug(X), debugln('...'),
    delta_numfacts(-1),
    incr_retract(X),
    fail.
...
try_retract_real(X) :-
    %debug('Asserting '), debug(X), debugln('...'),
    delta_numfacts(1),
    incr_asserta(X),
    fail.
...
debugClasses :-
    ...
    find(From, From2),
    (From = From2 -> fail;
     (debug('WARNING: '), debug(From), debug(' should be a class representative in '),
      debug(OldTerm), debug(' argument '), debug(Index),
      debug(' but '), debug(From2), debugln(' is the rep.'))).
...
mergeClasses(M1, M2) :-
    ...
    union(M1, M2),
    find(M1, NewRep),
    %debug('Merging method '), debug(M1), debug(' previously in class '), debug(S1),
    %debug(' with method '), debug(M2), debug(' previously in class '), debug(S2),
    %debug(' into the new class '), debugln(NewRep),
    (NewRep=S1 -> OldRep=S2; OldRep=S1),
    forall(fixupClasses(OldRep, NewRep, OldTerm, NewTerm),
           (debug('Retracting '), debug(OldTerm),
            debug(' and asserting '), debug(NewTerm), debugln(' ...'),
            % Now that all the conditions have been met, retract OldTerm and assert NewTerm.
            try_retract(OldTerm),
            try_assert(NewTerm))
          ),
...
% This routine is a helper to ensure that find() works at the interactive prompt as expected.
doit :-
    makeAllObjects,
    mergeClassesRepeatedly.
...
reasoningLoop :-
    ...
    reasonForwardAsManyTimesAsPossible,
    if_(sanityChecks,
        debugln('Constraint checks succeeded, guess accepted!'),
        (debugln('Constraint checks failed, retracting guess!'), fail)
       ),
    reasoningLoop.
...
checkCompleteness :-
    debug('Checking for completeness...'),
    not(completeClassHasNoSpecificBase),
    debugln('passed').

solve :-
    makeAllObjects,
    debugln('Initial reasoning'),
    reasonForwardAsManyTimesAsPossible,
    debugln('Starting reasoningLoop'),
    reasoningLoop
    ;
    writeln('No complete solution was found!').
...
solve(X) :-
    ...
    reasoningLoop
    ;
    %(writeln('No complete solution was found!'), fail).
    writeln('No complete solution was found!').
