...
reasonMethod_G(Method) :-
    factVFTableEntry(_VFTable, _Offset, Method).
...
reasonConstructor(Method) :-
    validVBTableWrite(_Insn, Method, _Offset, _VBTable).
...
reasonNOTConstructor_G(Method) :-
    ...
    funcOffset(_, OtherConstructor, Method, 0),
    % The caller is known to be a constructor.
    factConstructor(OtherConstructor),
    % The caller is already known to have a VFTable.
    factVFTableWrite(_Insn1, OtherConstructor, 0, _VFTable1),
...
% overwriting tables in the "normal" direction.
...
% PAPER: NA
reasonVFTableEntry(VFTable, Offset, Method) :-
...
reasonVFTableEntry(VFTable, 0, Method) :-
    factVFTable(VFTable),
    possibleVFTableEntry(VFTable, 0, Method).

% Because there is a larger valid VFTable entry in the same VFTable.
% PAPER: Larger-VFTableEntry
reasonVFTableEntry(VFTable, Offset, Address) :-
    factVFTableEntry(VFTable, ExistingOffset, _Method),
...
reasonNOTVFTableEntry(VFTable, Offset, Method) :-
    or([reasonNOTVFTableEntry_A(VFTable, Offset, Method),
        reasonNOTVFTableEntry_B(VFTable, Offset, Method),
        reasonNOTVFTableEntry_C(VFTable, Offset, Method),
        reasonNOTVFTableEntry_D(VFTable, Offset, Method),
        reasonNOTVFTableEntry_E(VFTable, Offset, Method)
      ]).

% Because it has already been proved not to be.
% PAPER: NA
reasonNOTVFTableEntry_A(VFTable, Offset, Method) :-
    factNOTVFTableEntry(VFTable, Offset, Method).

% Because the same address is known to already be a valid entry in a different table, and
% virtual function tables can not overlap.
% XXX: Why doesn't this use factVFTableEntry?
% PAPER: Overlap-NotVFTableEntry
reasonNOTVFTableEntry_B(VFTableAddress, Offset, Method) :-
    factVFTable(VFTableAddress),
    possibleVFTableEntry(VFTableAddress, Offset, Method),
    Offset \= 0,
...
reasonNOTVFTableEntry_C(VFTable, Offset, Method) :-
    ...
    possibleVFTableEntry(VFTable, ComputedOffset, OtherMethod),
    factNOTVFTableEntry(VFTable, ComputedOffset, OtherMethod).
...
reasonNOTVFTableEntrySet(VFTable, Set) :-
    factVFTable(VFTable),
    setof(Offset, Method^reasonNOTVFTableEntry(VFTable, Offset, Method), Set).
...
reasonVFTableSizeGTE(VFTable, Size) :-
    ...
    setof(S, M^factVFTableEntry(VFTable, S, M), Set),
...
% PAPER: VSize-0
% Paper doesn't use likelyVFTableEntry...
reasonVFTableSizeLTE(VFTable, Size) :-
    % Ed says: By prefixing M^ we tell setof NOT to case on M.
    % If we leave M as _, it will case on different values of M!
...
% --------------------------------------------------------------------------------------------
:- table likelyVFTableEntry/3 as incremental.

% A likely VFTable entry is a possible VFTable entry that's not explicitly contradicted.  Used
% for filtering VFTable guesses and setting maximum sizes for VFTables.
% PAPER: XXX
likelyVFTableEntry(VFTable, Offset, Method) :-
    possibleVFTableEntry(VFTable, Offset, Method),
...
%    factVFTableEntry(BaseVFTable, _BaseVFTableOffset, Method),
%    factVFTableEntry(DerivedVFTable, _DerivedVFTableOffset, Method),
%    iso_dif(DerivedVFTable, BaseVFTable),
%
...
% PAPER: Logic
reasonDerivedClass(DerivedClass, BaseClass, Offset) :-
...
reasonDerivedClass(DerivedClass, BaseClass, Offset) :-
    rTTIEnabled,
    rTTIValid,
    rTTIInheritsFrom(DerivedTDA, BaseTDA, _Attributes, Offset, _P, _V),
...
% PAPER: Relate-5
reasonClassHasUnknownBase_B(Class) :-
    % This should really be factVBTableWrite (see comment on reasonConstructor).
    validVBTableWrite(_Insn, Constructor, _Offset, _VBTable),
...
reasonClassHasUnknownBase_C(DerivedClass) :-
    % And there's a method that appears in two different VFTables...
    factVFTableEntry(AncestorVFTable, _AncestorVFTableOffset, Method),
    factVFTableEntry(DerivedVFTable, _DerivedVFTableOffset, Method),
...
reasonClassCallsMethod_B(Class1, Method2) :-
    ...
    factVFTableEntry(VFTable, _TableOffset, Method2),
    iso_dif(Method1, Method2),
...
% --------------------------------------------------------------------------------------------
:- table certainConflictedVirtualMethod/3 as incremental.

% It is certain that the method is a "conflicted virtual method".  A conflicted virtual method
% is one that appears in a virtual function table entry for the base class and the derived
% class.  The significance of this observations is that is certain that the method is NOT on
% the derived class.  We can't prove that it's on the base class until we're certain that the
% base class has no base classes.  This rule is a fact of it's own because it's useful for
% filtering
% PAPER: Merging-10
certainConflictedVirtualMethod(DerivedConstructor, BaseConstructor, Method) :-
    factConstructor(DerivedConstructor),
    factConstructor(BaseConstructor),
    find(DerivedConstructor, DerivedClass),
    find(BaseConstructor, BaseClass),

    % There's a derived class base class relationship.
    factDerivedClass(DerivedClass, BaseClass, ObjectOffset),
    % And it's certain to be on the base class virtually.
    factVFTableEntry(BaseVFTable, _BaseVTableOffset, Method),
    factVFTableWrite(_BaseInsn, BaseConstructor, ObjectOffset, BaseVFTable),
    % And it's also certain to be on the derived class virtually.
    factVFTableEntry(DerivedVFTable, _DerivedVTableOffset, Method),
    factVFTableWrite(_DerivedInsn, DerivedConstructor, ObjectOffset, DerivedVFTable).

certainConflictedVirtualMethodSet(Method, Set) :-
    setof([C1, C2], certainConflictedVirtualMethod(C1, C2, Method), Set).

% --------------------------------------------------------------------------------------------
% These rules are "helpers" designed to make the code easier to read, but they don't fit well
% into our existing naming scheme. :-(

% The methods are certain to be assigned to the exact same class.
% PAPER: NA
methodsOnSameClass(Method1, Method2, Class) :-
    find(Method1, Class),
    find(Method2, Class).

% The methods are certain to NOT be assigned to the exact same class.
% PAPER: NA
methodsNOTOnSameClass(Method1, Method2) :-
    iso_dif(Method1, Method2),
    find(Method1, Class1),
    find(Method2, Class2),
    iso_dif(Class1, Class2).

% For a little more performance, we could define some combination rules that takes three
% parameters and test for various combinations without computing find() more than once, but so
% far there doesn't seem to be any evidence that is beneficial.
...
%    find(Constructor, Class),
%    factClassHasNoBase(Class),
%    factVFTableEntry(VFTable, _VFTableOffset, Method),
%    debug('************** reasonMergeClasses('),
%    debug(Constructor), debug(', '),
%    debug(Method), debugln(').').

% Because the method occurs in a VFTable of a class that has no base.  Implement: This rule
% might be cleaner if it was expressed without ConflictedVirtualMethod.
% PAPER: Merging-3
reasonMergeClasses_B(Constructor, Method) :-
    certainConflictedVirtualMethod(_DerivedConstructor, Constructor, Method),
    not(purecall(Method)), % Never merge purecall methods into classes.
    find(Constructor, Class),
    factClassHasNoBase(Class),
    methodsNOTOnSameClass(Constructor, Method),
...
% PAPER: Merging-2
reasonMergeClasses_C(Class, Method) :-
    factClassCallsMethod(Class, Method),
    not(purecall(Method)), % Never merge purecall methods into classes.
...
reasonMergeClasses_D(Method1, Method2) :-
    ...
    % Never merge purecall methods into classes.
    not(purecall(Method1)),
    not(purecall(Method2)),
...
reasonMergeClasses_E(Class1, Class2) :-
    factDerivedClass(DerivedClass, Method1, ObjectOffset),
    factDerivedClass(DerivedClass, Method2, ObjectOffset),
    iso_dif(Method1, Method2),
    % Never merge purecall methods into classes.
    not(purecall(Method1)),
    not(purecall(Method2)),
...
reasonMergeClasses_H(DerivedClass, MethodClass) :-
    ...
    % There's an entry in the derived vftable that's to bug to be in the base vftable.
...
reasonNOTMergeClasses_E(Class1, Class2) :-
    ...
    iso_dif(Class1, Class2),
    factVFTableWrite(_Insn1, Constructor1, ObjectOffset, VFTable1),
    factVFTableWrite(_Insn2, Constructor2, ObjectOffset, VFTable2),
...
reasonSharedImplementation(Method, Class1) :-
    ...
    iso_dif(VFTable1, VFTable2),
    factVFTableEntry(VFTable1, _, Method),
    factVFTableEntry(VFTable2, _, Method),
...
:- table certainMemberOnConstructorClass/3 as incremental.
...
certainMemberOnConstructorClassSet(Constructor, Set) :-
    setof(Offset, Size^certainMemberOnConstructorClass(Constructor, Offset, Size), Set).

% --------------------------------------------------------------------------------------------
:- table certainMemberNOTOnExactConstructorClass/3 as incremental.
...
certainMemberNOTOnExactConstructorClass(Constructor, Offset, Size) :-
    certainMemberOnConstructorClass(Constructor, Offset, Size),
    find(Constructor, Class),
...
% --------------------------------------------------------------------------------------------
:- table certainMemberOnExactConstructorClass/3 as incremental.

certainMemberOnExactConstructorClass(Constructor, Offset, Size) :-
    certainMemberOnConstructorClass(Constructor, Offset, Size),
    % Exclude members that are actually on the embedded objects or base classes.
    not(certainMemberNOTOnExactConstructorClass(Constructor, Offset, Size)).

certainMemberOnExactConstructorClassSet(Constructor, Set) :-
    setof(Offset, Size^certainMemberOnExactConstructorClass(Constructor, Offset, Size), Set).
...
reasonClassSizeGTE_E(Class, Size) :-
    factMethod(Method),
    find(Method, Class),
    validMethodMemberAccess(Insn, Method, MemberOffset, MemberSize),
