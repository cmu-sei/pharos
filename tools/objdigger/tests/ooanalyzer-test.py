...
def test_ground_path(args):
    return os.path.join(args.git_dir, "tools", "objdigger", "tests" , args.testcase[0])

def test_output_path(args):
    return os.path.join(args.build_dir, "tools", "objdigger", "tests" , args.testcase[0])
...
def build_symbol_map(symbols_file):
    ...
    for l in lines:
        ...
        (addr, isthunk, idasymbol, symbol) = l.split('\t')
...
def run_ooanalyzer_test(args):
    ...
    tool_path = os.path.join(args.build_dir, "tools", "objdigger", "objdigger")
    ...
    previous_path = test_ground_path(args) + ".previous"
    diff_path = test_output_path(args) + ".run_diff"
    diff_cmd = "diff %s %s" % (previous_path, named_path)
    if not args.verbose:
    ...
    if rc != 0:
        if not args.quiet:
            report_error(args, "", "ooanalyzer diff rc=%d" % rc)
        failed += 1
...
def run_prolog_test(args):
    ...
    update_rules(args)
    rules_path = os.path.join(args.git_dir, "prolog", "oorules")
    os.chdir(rules_path)

    xsb_path = args.xsb_path
    assertions = "[rulerun],run('%s')." % ground_facts_path
...
def compare_prolog_answers(args, gen_fact_path, gen_results_path):
    ...
    # A common occurrence is for the thisptr_hashes to change.
    if len(extra_facts) == len(missing_facts) and len(missing_facts) != 0:
        ...
        for fact in gen_fact_lines:
            new_gen_facts.add(re.sub('thisptr_[0-9]+', 'thisptr_xxx', fact))
        new_ground_facts = set()
        for fact in ground_fact_lines:
            new_ground_facts.add(re.sub('thisptr_[0-9]+', 'thisptr_xxx', fact))
        ...
        if len(new_extra_facts) == 0 and len(new_missing_facts) == 0:
            if not args.quiet:
                print >>sys.stderr, "WARNING: %s ooanalyzer-prolog facts this_ptrs changed" % (testname)
...
def accept_ooanalyzer_test(args):
    named_path = test_output_path(args) + ".named"
    previous_path = test_ground_path(args) + ".previous"

    if args.commands:
        print "cp %s %s" % (named_path, previous_path)

    if not args.review:
        shutil.copyfile(named_path, previous_path)
