...
A bitmask that, when anded with the B<exact_bytes>, represent the
bytes that generate the B<pic_hash> when the MD5 is calculated.
...
A bitmask that, when anded with the B<cf_exact_bytes>, represent the
bytes that generate the B<pic_hash> when the MD5 is calculated.
