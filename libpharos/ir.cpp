...
namespace {
  ...
  // This is a hack so that undefined values do not cause z3 exceptions in spacer
  bool is_register_or_unspecified(Register r) {
    return is_register (r) || (r->flags() & SymbolicExpr::Node::UNSPECIFIED);
  }
                     ...
                     [&regs](const Stmt &stmt) {
                       ...
                       std::for_each (exps.begin (), exps.end (),
                                      [&regs](const IRExprPtr &exp) {
                                        ...
                                        boost::copy (allvars
                                                     | boost::adaptors::filtered (is_register_or_unspecified),
...
namespace pharos {
  namespace ir {
    ...
    IR IR::get_ir(const FunctionDescriptor* fd) {
      ...
      static IRRegisterStatePtr rstate = IRRegisterState::instance(protoval, regdict, fd->ds.get_arch_bits ());
